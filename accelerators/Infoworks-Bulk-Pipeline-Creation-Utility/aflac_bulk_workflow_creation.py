...
class BulkWorkflow():
    ...
    def prepare_pipeline_name_to_id_mappings(self, domain_id):
        ...
        for pipeline in pipelines:
            try:
                self.pipeline_name_to_id_mappings[pipeline["name"].lower()] = (
                pipeline["id"], pipeline["active_version_id"], pipeline["versions_count"])
    ...
    def prepare_workflow_model(self, domain_id, tg_id, tables):
        ...
        for index, table in enumerate(tables):
            edge_temp_list = []
            export_edge_temp_list = []
            dummy_node_edge_list=[]
            ...
            export_pipeline_name = self.configuration_json.get("export_pipeline_name_prefix", "") + "_" + table_name
            pipeline_id, pipeline_active_version_id, version = self.pipeline_name_to_id_mappings[pipeline_name.lower()]
            export_pipeline_id, export_pipeline_active_version_id, export_pipeline_version = self.pipeline_name_to_id_mappings[export_pipeline_name.lower()]
            pipeline_node_id = "PB_" + str(4000 + index)
            export_pipeline_node_id = "PB_" + str(5000 + index)
            ...
            tasks_temp = [{
                ...
                "is_group": False,
                "task_type": "pipeline_build",
                "task_id": export_pipeline_node_id,
                "location": f"195.1999969482422 {longitude}",
                "title": f"{export_pipeline_name}",
                "description": "",
                "task_properties": {
                    "task_subtype": "pipeline",
                    "pipeline_id": export_pipeline_id,
                    "version_id": export_pipeline_active_version_id,
                    "version": int(export_pipeline_version),
                    "pipeline_parameters": []
                },
                "run_properties": {
                    "trigger_rule": "all_success",
            ...
            export_edge_temp = {
                "from_task": pipeline_node_id,
                "to_task": export_pipeline_node_id,
                "category": "LINK",
                "task_id": -counter - 2
            }
            dummy_node_edge ={
                "from_task": export_pipeline_node_id,
                "to_task": "DM_D24F",
                "category": "LINK",
                "task_id": -counter - 3
            }
            edge_temp_list.append(edge_temp)
            export_edge_temp_list.append(export_edge_temp)
            dummy_node_edge_list.append(dummy_node_edge)
            base_model["tasks"].extend(tasks_temp)
            base_model["edges"].extend(edge_temp_list)
            base_model["edges"].extend(export_edge_temp_list)
            base_model["edges"].extend(dummy_node_edge_list)
            counter += 2
        return base_model

    def create_workflow_if_not_exists(self, domain_id, workflow_name, workflow_body):
        try:
            workflow_details_response = self.iwx_client.get_list_of_workflows(domain_id, params={
            ...
            if workflow_details == []:
                ...
                response = self.iwx_client.update_workflow(workflow_id=workflow_detail["id"], domain_id=domain_id,
    ...
    def bulk_workflow_creation(self):
        ...
        for table_group in table_groups:
            ...
            tg_details_response = self.iwx_client.get_table_group_details(source_id=self.source_id,
                                                                          tg_id=table_group["id"])
            ...
            if tg_details:
                ...
                res = self.create_workflow_if_not_exists(domain_id=self.domain_id,
                                                         workflow_name=f"{workflow_name_prefix}_" + tg_details["name"],
