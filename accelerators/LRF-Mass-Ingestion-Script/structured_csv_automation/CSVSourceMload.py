...
from helper_utilities.utils import print_success,print_info, print_error, is_int, is_float, set_src_advanced_config, create_src_extensions, dump_table_details
import numpy as np
...
class CSVSourceMload(FileBasedIWXSource):
    ...
    def configure_tables(self, source_id, table_id, table_name, table_config, snowflake_source_id):
        ...
        try:
            ...
            if snowflake_source_id is not None:
                print("getting snowflake details")
                ...
                try:
                    ...
                    if client_response["result"]["status"] == "success":
                        ...
                        print_error(f"Unable to fetch column details from source {snowflake_source_id}.Skipping the metadata configurations for {table_name}")
                        self.logger.error(f"Unable to fetch column details from source {snowflake_source_id}.Skipping the metadata configurations for {table_name}")
                        self.skipped_tables.append((table_name,f"Unable to fetch column details from source {snowflake_source_id}.",None,None))
            ...
            for lrf_column in lrf_column_names:
                ...
                if function_mappings.get(column_name_in_targetdb, column_name_in_lrf).startswith(
                    ...
                    if nvl2_contents.startswith("to_date") or nvl2_contents.startswith("to_timestamp"):
                        ...
                        nvl2_contents = nvl2_contents.replace(format_val,
                                                              self.teradata_spark_mappings.get(format_val.lower(),
                                                                                               second_val))
            ...
            new_columns_to_add = \
                self.table_schema_df.query(f"TABLE_NAME_JOIN_COL == '{table_name}'").fillna('')['NEW_COLUMNS'].tolist()[0]
                ...
                for column_data in new_columns_to_add:
                    ...
                    for k, v in column_data.items():
                        ...
                        if v.lower().startswith('lit'):
                            ...
                            params = [{"value": nvl_contents},{"value":nvl2_datatype_mapping.get(temp["target_sql_type"])}]
...
def parallelized_file_mapping_configuration(obj: CSVSourceMload, table, source_id, snowflake_source_id,
    ...
    try:
        ...
        print_error("Could not find the table entry in table_schema.csv.Skipping..")
        LRF_Path = None
    if LRF_Path is not None:
        ...
        try:
            delimiter = obj.table_schema_df.query(f"TABLE_NAME_JOIN_COL == '{table_name}'").fillna('')['Delimiter'].tolist()[0]
            if delimiter.strip() == "Fixed-width":
                return f"{table_name} File Mappings skipped since its Fixed width"
            else:
                file_mappings_body["configuration"]["source_file_properties"]["column_separator"] = delimiter.strip().split(" ")[0]
        except (KeyError, IndexError) as e:
            print_error("Failed to find the Delimiter column in table_schema.csv. Defaulting to default delimiter from configurations.json")
            print(str(e))
        ...
        try:
            table_schema = obj.table_schema_df.query(f"TABLE_NAME_JOIN_COL == '{table_name}'").fillna('')['DB Name'].tolist()[0]
            ...
            try:
                table_schema = \
                    obj.table_schema_df.query(f"TABLE_NAME_JOIN_COL == '{table_name}'").fillna('')[
                        'TARGET_SCHEMA_NAME'].tolist()[0]
            except (KeyError, IndexError) as e:
                # traceback.print_exc()
                print(str(e))
                print(
...
def main():
    ...
    if refresh_token == "":
        ...
        exit(-100)
    src_obj = CSVSourceMload(refresh_token=refresh_token, host=host, port=port, protocol=protocol)
    ...
    if snowflake_source_id is None:
        ...
        exit(-100)
    table_schema_path = args.get("table_schema_file_path","./table_schema.csv")
    ...
    #table_names_list = [i.replace("#","_") for i in table_names_list]
