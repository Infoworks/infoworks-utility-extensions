...
def main():
                        ...
                        try:
                            ...
                            for item in re.split(r',(?![^()]*\)|\s*FORMAT\s+)', insert_sql):
                                ...
                                if len(temp_search_result) > 0:
                                    if len(temp_search_result) > 1:
                                        lrf_col_name=":"+teradata_column_name
                                    else:
                                ...
                                if lrf_col_name is not None and len(temp_search_result) >= 1:
                                    #print("temp_search_result:",temp_search_result)
                                ...
                                if lrf_col_name is not None and not lrf_column_details.strip() == lrf_col_name.strip():
                                    ...
                                    #remove any other LRF columns other than teradata column
                                    for column in temp_search_result:
                                        lrf_column_details_removed_colon = lrf_column_details_removed_colon.replace(f"{column}",f"{column.strip(':')}")
    ...
    print("resultant_df before merge", resultant_df.info())
