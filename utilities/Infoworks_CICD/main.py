...
# keep original required set (no requirements.txt introduced)
required = {'infoworkssdk==5.1', 'tabulate==0.9.0'}
import pkg_resources

# original-style detection + install (kept as close to original)
    ...
    while True:
        try:
            ...
            if entity_type == "pipeline":
                ...
                # use splitext to remove extension safely and then extract name after '#'
                pipeline_name = os.path.splitext(pipeline_file_name)[0].split("#")[-1]
                # remove leading "pipeline_" prefix only if present
                if pipeline_name.startswith("pipeline_"):
                    pipeline_name = pipeline_name[len("pipeline_"):]
                # guard retrieval of overall status
                try:
                    overall_status = overall_steps_status[-1][1]
                except Exception:
                    overall_status = "UNKNOWN"
                with global_lock:
                    overall_pipelines_report_list.append((task['domain_name'], pipeline_name, overall_status,
            ...
            # always mark task done to avoid join() blocking forever
            try:
                q.task_done()
            except Exception:
                pass
...
if __name__ == '__main__':
    ...
    # === Make source upload optional ===
    src_csv_path = os.path.join("configurations", "modified_files", "source.csv")
    if os.path.exists(src_csv_path) and os.path.getsize(src_csv_path) > 0:
        with open(src_csv_path, "r") as src_files_fp:
            for src_file in src_files_fp.readlines():
                stripped = src_file.strip()
                if not stripped:
                    continue
                src_args = {"entity_type": "source",
                            "source_config_path": os.path.join("configurations/source", stripped)}
        ...
        print("No source.csv found or file empty — skipping source upload (was previously mandatory).")
    ...
    if maintain_lineage == 'true':
        ...
        for item in topological_sort_grouping(graph):
            for node in item:
                # compare as strings, not identity
                if str(node) != "root":
                    # use public nodes API
                    pipeline_file = graph.nodes[node].get('filename', '').strip()
                    if not pipeline_file:
                        print(f"Warning: node {node} missing filename, skipping")
                        continue
        ...
        job_queue.join()
        # print a readable summary similar to original
        if overall_pipelines_report_list:
            ...
            overall_pipelines_report_df['RESPONSE'] = overall_pipelines_report_df['RESPONSE'].apply(
                lambda tup: "\n".join([f"{i[0]}: ({i[1]}, {i[2]})" for i in tup]) if isinstance(tup, (list, tuple)) else str(tup))
            print(overall_pipelines_report_df.to_markdown())

        print('*** Done with Pipeline Configurations ***')

    # === PIPELINE GROUPS ===
    pipeline_grp_csv = os.path.join("configurations", "modified_files", "pipeline_group.csv")
    if os.path.exists(pipeline_grp_csv):
        with open(pipeline_grp_csv, "r") as pipeline_grp_file_fp:
            for pipeline_grp_file in pipeline_grp_file_fp.readlines():
                pipeline_grp_name = pipeline_grp_file.strip()
                if not pipeline_grp_name:
                    continue
                ...
                pipeline_grp_args = {"entity_type": "pipeline_group",
                                     "pipeline_group_config_path": os.path.join("configurations/pipeline_group",
                                                                              pipeline_grp_name),
                                     "domain_name": domain_name}
        ...
        print("No pipeline_group.csv found — skipping pipeline_group step.")
    ...
    if maintain_lineage == 'true':
        ...
        for item in topological_sort_grouping(graph):
            for node in item:
                if str(node) != "root":
                    wf_name = graph.nodes[node].get('filename', '').strip()
                    if not wf_name:
                        print(f"Warning: workflow node {node} missing filename, skipping")
                        continue
