...
class AdhocMetricsReport:
            ...
            for workflow in workflows:
                ...
                for task in tasks:
                    if task["task_type"]=="bash_command_run":
                        ...
                        temp["bash_node"]=task
                        temp["custom_image_url"]=task.get("task_properties",{}).get("advanced_configurations",{}).get("k8_image_url","")
                        ...
                        if matches:
                            ...
                            pass
                        workflow_latest_run = self.iwx_client.get_list_of_workflow_runs(domain_id=domain["id"],workflow_id=workflow["id"],params={"limit":1,"sort_by":"created_at","order_by":"desc"},pagination=False)
    ...
    def extract_infoworks_artifact_creator(self):
        ...
        users_lookup={}
        for user in users:
            users_lookup[user["id"]]=(user.get("profile",{}).get("name",""),user.get("profile",{}).get("email",""))
    ...
    def extract_extension_report(self):
        ...
        for source_extension in source_extension_list:
            ...
            dependencies = self.iwx_client.get_list_of_source_extension_dependencies(source_extension_id=source_extension_id)
        ...
        resultant_df = extract_extensions_df.merge(extension_usage_df, how="left",left_on="id",right_on="extension_id")
    ...
    def extract_target_data_connections_report(self):
        ...
        for source in sources:
            tables = self.iwx_client.list_tables_under_source(source_id=source["id"] ,params={"filter":{"export_configuration":{ "$exists": True }}})
        ...
        resultant_df = target_data_connections_df.merge(pipelines_using_extension_df, how="left",left_on="id",right_on="data_connection_id")
    ...
    def extract_generic_source_types_usage_report(self):
        ...
        for generic_source in generic_source_types:
            ...
            temp={}
            generic_source_dependencies = self.iwx_client.get_list_of_generic_source_type_dependencies(generic_source_type_id=generic_source["id"])
