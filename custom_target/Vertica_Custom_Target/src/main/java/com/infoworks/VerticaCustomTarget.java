...
public class VerticaCustomTarget implements SparkCustomTarget, AutoCloseable {
    private Logger LOGGER = LoggerFactory.getLogger(VerticaCustomTarget.class);
    private String VERTICA_DRIVER_NAME = "com.vertica.jdbc.Driver";
    ...
    private Integer defaultLongVarcharSize;
    private Integer defaultVarcharSize;
    private Map<String, Integer> varcharColumnsMappings;
    private Map<String, Integer> longVarcharColumnsMappings;
    private boolean allStringColsAsLongVarchar = false;

    public VerticaCustomTarget() {
    }
    ...
    private void validateAndInitConfigs(UserProperties userProperties) throws IOException {
        ...
        if (this.driverName == null) {
            this.LOGGER.warn("Driver name is not set. Using default driver : {}. To set the driver name, set property : {}", this.VERTICA_DRIVER_NAME, "driver_name");
            this.driverName = this.VERTICA_DRIVER_NAME;
        }
        ...
        if (userProperties.getValue("create_schema_if_not_exists") != null) {
            this.shouldCreateSchemaIfNotExists = Boolean.parseBoolean(userProperties.getValue("create_schema_if_not_exists"));
        }
        ...
        if (userProperties.getValue("all_string_columns_as_long_varchar") != null) {
            this.allStringColsAsLongVarchar = Boolean.parseBoolean(userProperties.getValue("all_string_columns_as_long_varchar"));
        }

        if (userProperties.getValue("default_long_varchar_size") != null) {
            this.defaultLongVarcharSize = Integer.valueOf(userProperties.getValue("default_long_varchar_size"));
        }
        ...
        if (userProperties.getValue("long_varchar_columns") != null) {
            this.longVarcharColumnsMappings = this.getColumnMap(userProperties.getValue("long_varchar_columns"));
        }
    ...
    public void writeToTarget(Dataset dataset) throws ExtensionsProcessingException {
        JdbcDialects.registerDialect(new VerticaJDBCDialect());
        if (this.syncMode == VerticaCustomTarget.TargetSyncMode.overwrite) {
            ...
            this.overwrite(dataset, this.schema, this.stagingSchema, this.table,"overwrite");
        } else if (this.syncMode == VerticaCustomTarget.TargetSyncMode.append) {
            this.append(dataset, this.schema, this.table);
        } else {
    ...
    private void overwrite(Dataset dataset, String schemaName, String stagingSchema, String tableName, String mode) {
        ...
        if (this.primaryColumnExpr!=null && !this.isUserManagedTable && this.syncMode == VerticaCustomTarget.TargetSyncMode.overwrite) {
    ...
    private void setPrimaryKey(String schemaName, String targetTable, String pkExpr) {
        String renameTableStatement = String.format("ALTER TABLE %s.%s ADD CONSTRAINT pk PRIMARY KEY (%s) ENABLED", schemaName, targetTable, pkExpr);
    ...
    private void renameTable(String schemaName, String stagingSchema, String sourceTable, String targetTable, String mode) {
        String renameTableStatement = String.format("ALTER TABLE %s.%s RENAME TO %s", stagingSchema, sourceTable, targetTable);
        this.executeDDLStatement(renameTableStatement);
        if (mode.equalsIgnoreCase("overwrite") && !stagingSchema.equalsIgnoreCase(schemaName)) {
            String setSchemaStatement = String.format("ALTER TABLE %s.%s SET SCHEMA %s", stagingSchema, targetTable, schemaName);
            this.executeDDLStatement(setSchemaStatement);
        }
    }
    ...
    private String getMergeTableStatement(String targetTable, String deltaTable, List<String> tableColumns, List<String> naturalColumns) {
        ...
        String deltaTableAlias = "D";
        String joinExpression = this.getJoinExpression(targetTableAlias, deltaTableAlias, naturalColumns);
        String updateRowExpression = this.getUpdateRowExpression(targetTableAlias, deltaTableAlias, tableColumns);
        String insertColumnNamesExpression = String.join(",", tableColumns);
        String deltaColumnValuesExpression = this.getDeltaColumnValuesExpression(deltaTableAlias, tableColumns);
        String mergeTableStatement = String.format("MERGE INTO %s AS %s\n  USING %s AS %s \n    ON (%s)\n  WHEN MATCHED THEN\n    UPDATE SET %s\n  WHEN NOT MATCHED THEN\n    INSERT (%s)\n    VALUES (%s);", targetTable, targetTableAlias, deltaTable, deltaTableAlias, joinExpression, updateRowExpression, insertColumnNamesExpression, deltaColumnValuesExpression);
        return mergeTableStatement;
    }

    private String getJoinExpression(String targetTableAlias, String deltaTableAlias, List<String> naturalColumns) {
        List<String> columnJoinExpressionList = new ArrayList();
        Iterator var5 = naturalColumns.iterator();

        while(var5.hasNext()) {
            String naturalColumn = (String)var5.next();
            columnJoinExpressionList.add(String.format("%s.%s = %s.%s", targetTableAlias, naturalColumn, deltaTableAlias, naturalColumn));
        }

        return String.join(" and ", columnJoinExpressionList);
    }

    private String getUpdateRowExpression(String targetTableAlias, String deltaTableAlias, List<String> columns) {
        List<String> updateColumnExpressionList = new ArrayList();
        Iterator var5 = columns.iterator();

        while(var5.hasNext()) {
            String columnName = (String)var5.next();
            // updateColumnExpressionList.add(String.format("%s.%s = %s.%s", targetTableAlias, columnName, deltaTableAlias, columnName));
            updateColumnExpressionList.add(String.format("%s = %s.%s", columnName, deltaTableAlias, columnName));
        }

        return String.join(" , ", updateColumnExpressionList);
    }

    private String getDeltaColumnValuesExpression(String deltaTableAlias, List<String> TableColumns) {
        List<String> columnValueExpressionList = new ArrayList();
        Iterator var4 = TableColumns.iterator();

        while(var4.hasNext()) {
            String columnName = (String)var4.next();
            columnValueExpressionList.add(String.format("%s.%s", deltaTableAlias, columnName));
        }

        return String.join(",", columnValueExpressionList);
    }



    private void createSchemaIfNotExists(String schemaName) {
        String dropSchemaQuery = String.format("CREATE SCHEMA IF NOT EXISTS %s", schemaName);
    ...
    private boolean checkIfTableExists(String schemaName, String tableName) {
        String query = String.format("SELECT 1 FROM v_catalog.tables WHERE table_schema = '%s' AND table_name = '%s'", schemaName, tableName);
    ...
    private String getVarcharType(String columnName, String defaultSqlType) {
        ...
        // Check if all_string_cols_as_long_varchar is set
        if (this.allStringColsAsLongVarchar) {
            if (this.defaultLongVarcharSize!=null && this.defaultLongVarcharSize > 0) {
                return "LONG VARCHAR(" + this.defaultLongVarcharSize + ")";
            } else {
                return "LONG VARCHAR";
            }
        }
        ...
        if (this.longVarcharColumnsMappings!=null && this.longVarcharColumnsMappings.containsKey(columnName)) {
            int size = this.longVarcharColumnsMappings.get(columnName);
    ...
    private Map<String, String> getSpark2SQLDataTypeMapping() {
        ...
        dataTypeMapping.put("DoubleType", "DOUBLE PRECISION");
        ...
        dataTypeMapping.put("BinaryType", "VARBINARY");
